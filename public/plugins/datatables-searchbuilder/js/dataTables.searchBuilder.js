  ...
  var Criteria = /** @class */ (function () {
    function Criteria(table, opts, topGroup, index, depth, serverData) {
      ...
      this.dom = {
        ...
        delete: $$2('<button/>')
          ...
          .attr('type', 'button'),
        inputCont: $$2('<div/>').addClass(this.classes.inputCont),
        ...
        if (this.s.type.includes('date') || this.s.type.includes('time')) {
          ...
          for (var i = 0; i < this.s.value.length; i++) {
            if (
              this.s.value[i] &&
              this.s.value[i].length > 0 &&
              luxon().DateTime.fromFormat(this.s.value[i], this.s.dateFormat)
                .invalid === null
            ) {
          ...
          if (option.val() === _this.dom.condition.val()) {
            ...
            for (var _b = 0, _c = _this.dom.value; _b < _c.length; _b++) {
              ...
              if (
                _this.s.filled &&
                val !== undefined &&
                _this.dom.inputCont.has(val[0]).length !== 0
              ) {
                _this.s.filled = false
                _this.s.dt.draw()
                _this.setListeners()
    ...
    Criteria.numFmtConditions = {
      '=': {
        ...
        search: function (value, comparison) {
          return (
            Criteria.parseNumFmt(value) === Criteria.parseNumFmt(comparison[0])
          )
        }
      },
      // eslint-disable-next-line sort-keys
